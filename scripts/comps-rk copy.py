...
lon, lat, pm25 = (
    gpm25["Easting"].values,
    gpm25["Northing"].values,
    gpm25["PM2.5"].values,
)

bins = gs.standard_bins((lat, lon), latlon=False)
bin_c, vario = gs.vario_estimate((lat, lon), pm25, bin_edges=bins, latlon=False)


model = gs.Spherical(latlon=False, dim=2)
# para, pcov, r2  = model.fit_variogram(bin_c, vario, return_r2=True)
...
dk = gs.krige.Detrended(
    model=model, cond_pos=(lat, lon), cond_val=pm25, trend=trend, fit_variogram=True
)
...
startTime = datetime.now()
fld_dk = dk((gridy, gridx), return_var=False, mesh_type="structured")
...
# # %%
polygons, values = pixel2poly(gridx, gridy, RK_pm25, resolution)
