...
# - As we are using regression krigging, we solve the residuals based on a trend model fitted of based on the relation of PM2.5 to Elevation.
#
# - The trend in this case is define by a simple linear model regression of PM2.5 and elevation
#
# - After the trend model is fitted, we will apply ordinary kriging (OK) to the residuals.
#
...
# %%
ds = cfcompliant(str(data_dir) + f"/dispersion.nc")
ds = ds.sel(time="2021-07-16T22:00:00", z=0)
ds.coords["x"] = ds.LONG.values[0, :]
ds.coords["y"] = ds.LAT.values[:, 0]

# ds[var] = xr.where(ds[var]<0, 0, ds[var])
# ds[var] = xr.where(ds[var]>1, 1, ds[var])
...
var_points = ds[var].interp(x=x, y=y, method="linear")
# print(var_points)
