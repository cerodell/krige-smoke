...
class Channel:
    ...
    def _setup(self) -> None:
        ...
        self.location_type: Optional[str] = self.channel_data.get("DEVICE_LOCATIONTYPE")
        ...
        self.current_pm1_0_cf_1: Optional[float] = self._safe_float("pm1_0_cf_1")
        self.current_pm2_5_cf_1: Optional[float] = self._safe_float("pm2_5_cf_1")
        self.current_pm10_0_cf_1: Optional[float] = self._safe_float("pm10_0_cf_1")
        ...
        self.current_pm10_0_atm: Optional[float] = self._safe_float("pm10_0_atm")

        # Statistics
        self.pm2_5stats: Optional[dict] = (
            json.loads(self.channel_data["Stats"])
            if "Stats" in self.channel_data
            else None
        )
        self.m10avg: Optional[float] = (
            self.pm2_5stats.get("v1") if self.pm2_5stats else None
        )
        self.m30avg: Optional[float] = (
            self.pm2_5stats.get("v2") if self.pm2_5stats else None
        )
        self.h1ravg: Optional[float] = (
            self.pm2_5stats.get("v3") if self.pm2_5stats else None
        )
        self.h6ravg: Optional[float] = (
            self.pm2_5stats.get("v4") if self.pm2_5stats else None
        )
        self.d1avg: Optional[float] = (
            self.pm2_5stats.get("v5") if self.pm2_5stats else None
        )
        self.w1avg: Optional[float] = (
            self.pm2_5stats.get("v6") if self.pm2_5stats else None
        )
        self.last_modified_stats: Optional[datetime] = None
        last_mod = (
            self.pm2_5stats.get("lastModified") if self.pm2_5stats is not None else None
        )
        if last_mod is not None:
            self.last_modified_stats = datetime.utcfromtimestamp(int(last_mod) / 1000)
        self.last2_modified: Optional[int] = (
            self.pm2_5stats.get("timeSinceModified")
            if self.pm2_5stats is not None
            else None
        )

        # ThingSpeak IDs, if these are missing do not crash, just set to None
        try:
            self.tp_primary_channel: Optional[str] = self.channel_data[
                "THINGSPEAK_PRIMARY_ID"
            ]
            self.tp_primary_key: Optional[str] = self.channel_data[
                "THINGSPEAK_PRIMARY_ID_READ_KEY"
            ]
            self.tp_secondary_channel: Optional[str] = self.channel_data[
                "THINGSPEAK_SECONDARY_ID"
            ]
            self.tp_secondary_key: Optional[str] = self.channel_data[
            ...
            self.thingspeak_secondary: Optional[
                thingspeak.Channel
            ] = thingspeak.Channel(
        ...
        self.hidden = False if self.channel_data.get("Hidden") == "false" else True
        # pylint: disable=simplifiable-if-expression
        self.flagged = True if self.channel_data.get("Flag") == 1 else False
        # pylint: disable=simplifiable-if-expression
        self.downgraded = True if self.channel_data.get("A_H") == "true" else False
        ...
        self.last_update_check: Optional[int] = self.channel_data.get("LastUpdateCheck")
    ...
    def created_date(self):
        ...
        created_at = datetime.strptime(
            data["channel"]["created_at"], "%Y-%m-%dT%H:%M:%SZ"
        )
        return created_at
    ...
    def get_all_historical_between(
        self,
        start_date: datetime,
    ...
    def get_historical_between(
        ...
        url = self._get_thingspeak_url(
            thingspeak_field, start_date, end_date, thingspeak_args
        )
