...
class SensorList:
    ...
    def get_all_data(self) -> None:
        ...
        if "results" not in data:
            ...
            raise ValueError(f"No sensor data returned from PurpleAir: {error_message}")
    ...
    def parse_raw_result(self, flat_sensor_data: dict) -> None:
        ...
        for child_sensor_id in child_map:
            ...
            if parent_sensor_id not in parent_map:
                ...
                continue
            channels = [parent_map[parent_sensor_id], child_map[child_sensor_id]]
    ...
    def generate_sensor_list(self) -> None:
        ...
        if self.parse_location:
            # pylint: disable=line-too-long
            print(
                "Warning: location parsing enabled! This reduces sensor parsing speed to less than 1 per second."
            )
    ...
    def to_dataframe(
        ...
        value_filter: Union[str, int, float, None] = None,
    ) -> pd.DataFrame:
        """
        ...
        try:
            sensor_data: pd.DataFrame = {
                "all": lambda: pd.DataFrame(
                    [s.as_flat_dict(channel) for s in self.all_sensors]
                ),
                "outside": lambda: pd.DataFrame(
                    [
                        s.as_flat_dict(channel)
                        for s in [
                ...
                "family": lambda: pd.DataFrame(
                    [
                        s.as_flat_dict(channel)
                        for s in [s for s in self.all_sensors if s.parent and s.child]
                    ]
                ),
                "column": lambda: self.filter_column(channel, column, value_filter),
            }[sensor_filter]()
        except KeyError as err:
            raise KeyError(f"Invalid sensor filter supplied: {sensor_filter}") from err
