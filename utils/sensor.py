...
class Sensor:
    ...
    def __init__(
        self, identifier: int, json_data: Optional[list] = None, parse_location=False
    ):
        self.data: Optional[list] = (
            json_data if json_data is not None else self.get_data(identifier)
        )

        # Validate the data we received
        if not self.data:
            raise ValueError(f"Invalid sensor: no configuration found for {identifier}")
        if not isinstance(self.data, list):
            raise ValueError(f"Sensor {identifier} created without valid data")

        self.parent_data: dict = self.data[0]
        self.identifier = self.parent_data["ID"]
        self.child_data: Optional[dict] = self.data[1] if len(self.data) > 1 else None
        self.parse_location: bool = parse_location
        self.thingspeak_data: dict = {}
        self.parent: Channel = Channel(
            channel_data=self.parent_data,
        )
        self.child: Optional[Channel] = (
            Channel(channel_data=self.child_data) if self.child_data else None
        )
    ...
    def get_data(self, identifier: int) -> Optional[list]:
        ...
        if channel_data and len(channel_data) == 1:
            ...
            raise ValueError(f"More than 2 channels found for {identifier}")
        return channel_data
    ...
    def get_field(self, field: int) -> None:
        ...
        # Primary
        self.thingspeak_data[field]["primary"]["channel_a"] = (
            json.loads(self.parent.thingspeak_primary.get_field(field=field))
            if self.parent.thingspeak_primary
            else None
        )
        self.thingspeak_data[field]["primary"]["channel_b"] = (
            json.loads(self.child.thingspeak_primary.get_field(field=field))
            if self.child and self.child.thingspeak_primary
            else None
        )

        # Secondary
        self.thingspeak_data[field]["secondary"]["channel_a"] = (
            json.loads(self.parent.thingspeak_secondary.get_field(field=field))
            if self.parent.thingspeak_secondary
            else None
        )
        self.thingspeak_data[field]["secondary"]["channel_b"] = (
            json.loads(self.child.thingspeak_secondary.get_field(field=field))
            if self.child and self.child.thingspeak_secondary
            else None
        )
    ...
    def as_list(self) -> List[Optional[dict]]:
        ...
        """
        return [self.parent.as_dict(), self.child.as_dict() if self.child else None]
